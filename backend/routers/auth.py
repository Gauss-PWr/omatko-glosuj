...
def get_user(db: Session, username: str):
    ...
    if user:
        return schemas.UserInDB(user_id = user.user_id, username=user.user_login, email=user.user_mail, password=user.user_password)
...
async def change_password(password_change_request: schemas.PasswordChangeRequest, user=Depends(get_current_user), db: Session = Depends(get_db)):
