...
export class Auth {
	...
	async login(code: string): Promise<Result<Session, AppError.UserNotFound>> {
		const codeHash = encode(code);
		const getUser = await this.db.query.users.findFirst({
			where: (t, { eq }) => eq(t.accessCode, codeHash),
			columns: {
				id: true,
				role: true
			}
	...
	async auth(
		...
		Result<{ userId: number; role: Role }, AppError.SessionNotFound | AppError.SessionExpired>
	> {
		const tokenHash = encode(token);
		...
		if (result.expiresAt.getTime() < authTime) return Err({ code: AppError.Code.SESSION_EXPIRED });

		return Ok({ userId: result.userId, role: result.role });
	}

	async logout(token: string): Promise<Result<undefined, AppError.SessionNotFound>> {
		const tokenHash = encode(token);
