...
const Login = (): ReactElement => {
    ...
    useEffect(() => {
        const login = async () => {
            ...
            const data = JSON.parse(localStorage.getItem('user_data') || JSON.stringify(initialData))
            if (data !== initialData) return

            try {
                const res = await axios.post(`${API_CALL_URL}/auth/token`, 
                JSON.parse(localStorage.getItem('user_data') || JSON.stringify(initialData)), 
    ...
    const handleSubmit = async (event: React.SyntheticEvent) => {
        ...
        try {
            const res = await axios.post(`${API_CALL_URL}/auth/token`, formData, {headers: {"content-type": "application/x-www-form-urlencoded"}})
            if (await res.status === 200) {
                dispatch({type: 'LOGIN', payload: {
                    ...formData,
                    token: {
            ...
            switch(error.response?.status){
                case 401: setIsDataCorect(false)
            }
        }
    }
